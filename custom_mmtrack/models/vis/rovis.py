...
from ..track_heads.rovis_head import tensor_deselect
...
def extract_feat_incl_args(self, batch_inputs, *args, **kwargs):
    # 'self' here refers to the detector instance
    x = self.backbone(batch_inputs, *args, **kwargs)
    if self.with_neck:
        x = self.neck(x)
    return x
...
class ROVIS(BaseMultiObjectTracker):
    ...
    def __init__(self,
        ...
        if detector is not None:
            ...
            #self.detector.extract_feat = extract_feat_incl_args
            self.detector.extract_feat = types.MethodType(extract_feat_incl_args, self.detector)
        ...
        #if not hasattr(self.train_cfg, "use_distill_loss"):
        #    self.train_cfg.use_distill_loss = False
        #self.use_fgs = self.train_cfg.get('use_fgs', False)


        self.test_cfg = test_cfg if test_cfg is not None else ConfigDict()
        if tracker is not None:
            self.tracker = MODELS.build(tracker)
        self.use_inference_setup = self.test_cfg.get('use_inference_setup', False)
    ...
    def add_track_queries_to_targets(self,
        ...
        for batch_idx, (prev_ind, data_sample) in enumerate(
            ...
            if add_false_pos:
                assert prev_masks.shape[1] > 0, f"prev_masks: {prev_masks.shape[1]}" #todo remove if issue is found
                    ...
                    if target_ind_matching[j]:
                        ...
                        try:
                            ...
                            if len(not_prev_out_ind) > 0:
                                ...
                                random_false_out_idx = random.randint(0, 100)
                    else:
                        if len(not_prev_out_ind) > 0:
                            # in rare cases the weights might be all zeros
                            random_false_out_idx = not_prev_out_ind.pop(
    ...
    def _calc_dist_loss(self, x_s, x_t, mask):
        ...
        #x_s_1 = x_s[1]
        #x_t_1 = x_t[1]
        #downsie_scale = [1/4, 1/2, 1, 2]
        goal_size = x_s[2].shape
        scale_factors = [4, 2, 1, 0.5]
        B, e, H, W = x_s[2].shape
        mask_H = x_s[2].shape[2]
        mask_W = x_s[2].shape[3]
        mask = mask.view(B, mask_H, mask_W).unsqueeze(1)

        for i, (feat_s, feat_t) in enumerate(zip(x_s, x_t)):

            B, e, H, W = feat_s.shape
            #kl_loss = F.kl_div(feat_s, feat_t, reduction='mean', log_target=True)

            #scaled_s = F.interpolate(feat_s, size=goal_size[-2:], mode='bilinear')
            #scaled_t = F.interpolate(feat_t, size=goal_size[-2:], mode='bilinear')

            #test01 = feat_s[0, 0, :5, :5].float().detach().cpu().numpy()
            #test02 = feat_t[0, 0, :5, :5].float().detach().cpu().numpy()

            #test01 = mask[0, 0, :5, :5].float().detach().cpu().numpy()
            upscaled_mask = F.interpolate(
                input=mask.float(),
                scale_factor=(scale_factors[i], scale_factors[i]),
                mode='nearest'
            )
            #test02 = upscaled_mask[0,0,  :10, :10].float().detach().cpu().numpy()
            upscaled_mask = upscaled_mask.bool().flatten(2).squeeze(1)
            ...
            kl_loss = 0
            for b in range(B):
                feat_s_b = feat_s[b, upscaled_mask[b]].unsqueeze(0)
                feat_t_b = feat_t[b, upscaled_mask[b]].unsqueeze(0)

                feat_s_b = F.log_softmax(feat_s_b / T, dim=1)
                feat_t_b = F.log_softmax(feat_t_b / T, dim=1)
                ...
                kl_loss = kl_loss + kl_loss_b
            kl_losses = kl_losses + (kl_loss / B)
            '''

            feat_s = F.log_softmax(feat_s / T, dim=1)
            feat_t = F.log_softmax(feat_t / T, dim=1)
    ...
    def loss(self, inputs: Dict[str, torch.Tensor], data_samples: SampleList,
        ...
        losses = dict()
        if self.detector.backbone.pruning_method == 'ivpp':
            self.detector.backbone.prev_x = None

            reff_img = inputs['reff_img'][:, 0]
            reff_x, prev_x, reff_loss = self.detector.extract_feat(reff_img,
                                        prev_x=None, need_loss=True, loss_name_prefix="reff_")
            losses.update(reff_loss)

            #ref_mask = inputs['ref_gt_p_mask'][:, 0] if 'ref_gt_p_mask' in inputs.keys() else None
            ref_x, prev_x, ref_loss = self.detector.extract_feat(ref_img,
                                       prev_x=prev_x, need_loss=True, loss_name_prefix="ref_")
            losses.update(ref_loss)

            #curr_mask = inputs['gt_p_mask'][:, 0] if 'gt_p_mask' in inputs.keys() else None
            x, prev_x, loss = self.detector.extract_feat(img,
                                       prev_x=prev_x, need_loss=True)
            losses.update(loss)
        ...
        if hasattr(self.detector,
            ...
            assert all_mask_preds[-1].shape[1] == 100, f"queries all_mask_preds: {all_mask_preds[-1].shape[1]}" #todo remove if issue is found
    ...
    def predict(self,
        ...
        # extract features
        if self.detector.backbone.pruning_method == 'ivpp' and frame_id == 0:
            self.detector.backbone.prev_x = None
        x = self.detector.extract_feat(img)
