...
class PackTrackInputs_MultiRefImgs(BaseTransform):
    ...
    def __init__(self,
                 ...
                 reduce_multiple_ref_imgs: bool = False,
                 sort_ref_imgs: bool = False,
        ...
        self.reduce_multiple_ref_imgs = reduce_multiple_ref_imgs
        self.sort_ref_imgs = sort_ref_imgs
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.reduce_multiple_ref_imgs and self.sort_ref_imgs:
            #reorder sampled frames
            sorted_indices = sorted(range(len(results['frame_id'])),
                                    key=lambda i: results['frame_id'][i],
                                    reverse=random.choice([True, False]))

            sorted_results = {}
            for key, value_list in results.items():
                # Use a list comprehension to reorder the current list
                # based on the `sorted_indices`.
                sorted_list = [value_list[i] for i in sorted_indices]
                sorted_results[key] = sorted_list
        ...
        if 'img' in results:
            imgs = results['img']
            if self.reduce_multiple_ref_imgs:
                #refff_img = imgs.pop(-1)
                #refff_img = refff_img[None, :].transpose(0, 3, 1, 2)
                #packed_results['inputs']['refff_img'] = to_tensor(refff_img.copy())

                reff_img = imgs.pop(-1)
                reff_img = reff_img[None, :].transpose(0, 3, 1, 2)
                packed_results['inputs']['reff_img'] = to_tensor(reff_img.copy())
            ...
            if ref_imgs is not None:
                ...
                packed_results['inputs'][f'{self.ref_prefix}_img'] = to_tensor(ref_imgs)
        ...
        if 'gt_ignore_flags' in results:
            ...
            if self.reduce_multiple_ref_imgs:
                #gt_ignore_flags.pop(-1)
                gt_ignore_flags.pop(-1)
            (key_gt_ignore_flags, ref_gt_ignore_flags) = self._cat_same_type_data(gt_ignore_flags)
        ...
        for key in self.meta_keys:
            ...
            if ref_img_metas is not None:
                if self.reduce_multiple_ref_imgs:
                    new_img_metas[f'{self.ref_prefix}_{key}'] = ref_img_metas[0]
                    #new_img_metas[f'{self.ref_prefix}_{self.ref_prefix}_{key}'] = ref_img_metas[1]
                else:
