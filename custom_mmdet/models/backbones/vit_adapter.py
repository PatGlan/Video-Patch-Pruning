...
class ViTAdapter(TIMMVisionTransformer):
    def __init__(self, pretrain_size=224, num_heads=12, conv_inplane=64, n_points=4, deform_num_heads=6,
                 ...
                 deform_ratio=1.0, add_vit_feature=True, use_extra_extractor=True, num_classes=80, init_cfg=None, *args, **kwargs):
        ...
        if init_cfg is not None:
            if init_cfg.type == 'Pretrained':
                ...
                #catch backbone-elements
                state_dict = {k.replace('detector.', '').replace('backbone.', ''): v for k, v in state_dict.items() if 'backbone.' in k}
                #state_dict = {k.replace('backbone.', ''):v for k, v in state_dict.items()}
                self.load_state_dict(state_dict, strict=True)
                logger.info(f"loaded checkpoint for VITAdapter from path: {self.init_cfg.checkpoint}")
            if hasattr(init_cfg, 'freeze') and init_cfg.freeze:
    ...
    def forward(self, x, ret_interm_results=False):
        ...
        if ret_interm_results:
            return tuple([f1, f2, f3, f4]), interm_x
