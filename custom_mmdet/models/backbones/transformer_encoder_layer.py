...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        if ret_res:
            if self.norm_first:
                res1 = self.drop_path1(self._sa_block(self.norm1(x), src_mask, src_key_padding_mask))
                x = x + res1
                res2 = self.drop_path2(self._ff_block(self.norm2(x)))
                x = x + res2

            else:
                res1 = self.drop_path1(self._sa_block(x, src_mask, src_key_padding_mask))
                x = self.norm1(x + res1)
                res2 = self.drop_path2(self._ff_block(x))
                x = self.norm2(x + res2)
            res = res1 + res2
            return x, res
